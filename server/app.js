...
app.post("/contact", async (req, resp) => {
    ...
    try {
        ...
        if (result1.success) {
            finalRequest = resp.json({ status: 200, message: "Form submitted successfully", data: result1 })
        } else {
            finalRequest = resp.json({ status: 400, message: "Form submission failed", error: result1 })
        }
    } catch (error) {
        finalRequest = resp.json({ status: 500, message: "Internal Server Error", error: error.message })
    }
    return finalRequest;
});
